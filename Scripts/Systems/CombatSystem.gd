...
static func simulate_round(state: BattleState) -> Dictionary:
	...
	var atk_defense = _calculate_total_defense(state.attacker) \
		* atk_formation.get("defense_modifier", 1.0)
	...
	if not commander.is_empty():
		...
		if not config.is_empty():
			for skill in config.get("skills", []):
				if skill.get("type", "") == "active" and round % skill.get("unlock_tier", 5) == 0:
...
static func _calculate_total_attack(army: Dictionary) -> float:
	...
	var military_manager = _get_military_manager()
	for unit_type in units:
		var count = units[unit_type].get("count", 0) if typeof(units[unit_type]) == TYPE_DICTIONARY else int(units[unit_type])
		var defn = military_manager.get_unit_def(unit_type) if military_manager else null
		if defn:
			total += count * defn.get("attack", 1)
		else:
			total += count
	var commander = army.get("commander_data", {})
	if not commander.is_empty():
		...
		var is_naval = _is_naval_army(army)
		if is_naval:
			total *= mods.get("naval_attack", 1.0)
	return total

static func _calculate_total_defense(army: Dictionary) -> float:
	var total := 0.0
	var units = army.get("units", {})
	var military_manager = _get_military_manager()
	for unit_type in units:
		...
		var defn = military_manager.get_unit_def(unit_type) if military_manager else null
		if defn:
			total += count * defn.get("defense", 1)
		else:
			total += count
	var commander = army.get("commander_data", {})
	if not commander.is_empty():
...
static func _apply_damage(army: Dictionary, power: float, defense: float) -> Dictionary:
	var losses = {}
	var effective_damage = max(1, power - defense * 0.5)
	var units = army.get("units", {})
	if units.is_empty():
		return losses
	...
	var damage_per_unit = effective_damage / total_units
	for unit_type in units:
		var count = units[unit_type].get("count", 0) if typeof(units[unit_type]) == TYPE_DICTIONARY else int(units[unit_type])
		var killed = mini(count, max(1, int(damage_per_unit * 0.1 * count)))
		if killed > 0:
