...
@onready var commander_panel = get_node_or_null("../CommanderPanel")
@onready var quest_panel = get_node_or_null("../QuestPanel")
@onready var event_panel = get_node_or_null("../EventPanel")
@onready var research_panel = get_node_or_null("../ResearchPanel")
@onready var military_panel = get_node_or_null("../MilitaryPanel")
@onready var trade_panel = get_node_or_null("../TradePanel")
@onready var ally_panel = get_node_or_null("../AllyPanel")
...
func _setup_resource_display() -> void:
	var display_resources = [
		...
		Globals.ResourceType.STONE,
		Globals.ResourceType.SATISFACTION,
	]
	...
	_setup_unit_count_display()

func _setup_unit_count_display() -> void:
	var hb = HBoxContainer.new()
	hb.size_flags_horizontal = SIZE_SHRINK_CENTER

	var icon = TextureRect.new()
	if ResourceLoader.exists("res://Assets/Textures/UI/sword.svg"):
		icon.texture = ResourceLoader.load("res://Assets/Textures/UI/sword.svg")
	icon.custom_minimum_size = Vector2(24, 24)
	hb.add_child(icon)

	var vbox = VBoxContainer.new()
	_unit_count_label = Label.new()
	_unit_count_label.text = "0"
	_unit_count_label.add_theme_color_override("font_color", Color(0.9, 0.2, 0.2))
	_unit_count_label.add_theme_font_size_override("font_size", 12)

	var name_label = Label.new()
	name_label.text = "نیرو"
	name_label.add_theme_color_override("font_color", Color(0.7, 0.7, 0.7, 0.7))
	name_label.add_theme_font_size_override("font_size", 9)

	vbox.add_child(_unit_count_label)
	vbox.add_child(name_label)
	hb.add_child(vbox)
...
func update_display() -> void:
	...
	if _unit_count_label:
		...
		for ut in units:
			var d = units[ut]
			total_units += int(d.get("count", 0)) if typeof(d) == TYPE_DICTIONARY else int(d)
		_unit_count_label.text = "%d" % total_units
...
func _on_unit_trained(_city_id: String, _unit_type: String, _count: int) -> void:
	update_display()
...
func _on_commander() -> void:
	...
	HapticManager.button_press()
	if commander_panel:
		commander_panel.visible = not commander_panel.visible
		if commander_panel.visible and commander_panel.has_method("open"):
...
func _on_quest() -> void:
	...
	if quest_panel:
		...
		if quest_panel.visible and quest_panel.has_method("open"):
			quest_panel.open()
...
func _on_event() -> void:
	...
	if event_panel:
		...
		if event_panel.visible and event_panel.has_method("open"):
			event_panel.open()
...
func _on_research() -> void:
	...
	if research_panel:
		...
		if research_panel.visible and research_panel.has_method("show_panel"):
			research_panel.show_panel()

func _on_military() -> void:
	AudioManager.play_button_click()
	HapticManager.button_press()
	if military_panel:
		military_panel.visible = not military_panel.visible
		if military_panel.visible and military_panel.has_method("open"):
			military_panel.open(GameState.selected_city_id)
...
func _on_diplomacy() -> void:
	...
	if trade_panel:
		...
		if trade_panel.visible and trade_panel.has_method("open"):
			trade_panel.open(GameState.selected_city_id)

func _on_alliance() -> void:
	AudioManager.play_button_click()
	HapticManager.button_press()
	if ally_panel:
		ally_panel.visible = not ally_panel.visible
